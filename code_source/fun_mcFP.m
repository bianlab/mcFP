...
while 1
    ...
    if i_last == size(kx,2)
        ...
        suptitle(['mcFP ' num2str(shiftguess_flag) ' iteration ' num2str(j)]);
    ...
    if shiftguess_flag == 1
        if exist('motionxyflag','var') == 1 && motionxyflag == 1
            ...
            for i = i_last_last+1:i_last
                clear error_now shift_all_x_temp shift_all_y_temp
                ...
                shift_all_x_temp = unique(round(motionxy_guess(i,1) + shift_all_x * RangeShink));
                shift_all_y_temp = unique(round(motionxy_guess(i,2) + shift_all_y * RangeShink));
